...
module Make (F : Cstubs.FOREIGN) = struct
  ...
  open F
    ...
    let fkeep_func_ptr = funptr (int64_t @-> int64_t @-> double @-> ptr void @-> returning (ptr int64_t))
    (* let cs_dl_fkeep = foreign "cs_dl_fkeep" (ptr cs_dl @-> ptr int64_t @-> ptr int64_t @-> int64_t @-> returning int64_t) *)
