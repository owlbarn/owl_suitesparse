...
let cs_dl_spfree a =
  Owl_suitesparse_ffi.CS.cs_dl_spfree a
  |> ignore
...
let cs_dl_spalloc ~m ~n ~nzmax ~values ~t =
  ...
  let a = Owl_suitesparse_ffi.CS.cs_dl_spalloc m n nzmax values t in
...
let cs_dl_entry ~a ~i ~j ~v = 
  ...
  let result = Owl_suitesparse_ffi.CS.cs_dl_entry a i j v in
...
let cs_dl_print ~a ~belief = 
  ...
  let result = Owl_suitesparse_ffi.CS.cs_dl_print a belief in
  let result = Int64.to_int result in
  Owl_suitesparse_exception.fail_on_nonzero result "cs_dl_print"


let cs_dl_compress a =
  let b = Owl_suitesparse_ffi.CS.cs_dl_compress a in
  Gc.finalise cs_dl_spfree b;
  b


let cs_dl_add ~a ~b ~alpha ~beta =
  let c = Owl_suitesparse_ffi.CS.cs_dl_add a b alpha beta in
...
let cs_dl_dupl a = 
  let result = Owl_suitesparse_ffi.CS.cs_dl_dupl a in
  let result = Int64.to_int result in
  Owl_suitesparse_exception.fail_on_nonzero result "cs_dl_dupl"


let cs_dl_transpose ~a ~values =
  let b = Owl_suitesparse_ffi.CS.cs_dl_transpose a values in
  Gc.finalise cs_dl_spfree b;
  b


let cs_dl_norm a =
  let result = Owl_suitesparse_ffi.CS.cs_dl_norm a in
  result


let cs_dl_lusol ~order ~a ~tol =
  let b_ptr = Ctypes.(allocate double 0.) in
  let result = Owl_suitesparse_ffi.CS.cs_dl_lusol order a b_ptr tol in
  let result = Int64.to_int result in
...
let cs_dl_qrsol ~order ~a =
  let b_ptr = Ctypes.(allocate double 0.) in
  let result = Owl_suitesparse_ffi.CS.cs_dl_qrsol order a b_ptr in
  let result = Int64.to_int result in
